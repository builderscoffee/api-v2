...
public class Configurations {
     ...
     */
    @SneakyThrows
    public <T> T readOrCreateConfiguration(net.md_5.bungee.api.plugin.Plugin plugin, Class<T> structure) {
        if (!structure.isAnnotationPresent(Configuration.class))
            throw new RuntimeException("Tried to load a non-annotated configuration");
     ...
     */
    @SneakyThrows
    public <T> void writeConfiguration(net.md_5.bungee.api.plugin.Plugin plugin, T configuration) {
        val structure = configuration.getClass();
        if (!structure.isAnnotationPresent(Configuration.class))
            throw new RuntimeException("Tried to load a non-annotated configuration");
